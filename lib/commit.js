...
var commit = function(cmdPrefix, msg, nopush) {
    exec(cmdPrefix + 'commit -m "' + msg + '"', function(err) {
        ...
        if (config['deploy'].rsync && config['deploy'].rsync.enable) {
            return deploy_rsync();
        }
...
function deploy_rsync() {
    ...
    var dir = path.resolve(cwd + '/' + config['dir'].site);
    var destdir = path.resolve(cwd + '/.deploy');
    var remotedir = config['deploy'].rsync.user + '@' + config['deploy'].rsync.host + ':' + config['deploy'].rsync.root;
    ...
    ncp(dir, destdir, ncpOpts, function(err) {
        ...
        rsync.execute(function(error, code, cmd) {
            // we're done
            console.log("Rsync Site updated.");
        });
    });
